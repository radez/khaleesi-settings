...
#
# written by REPLACE_ME@REPLACE_ME & REPLACE_ME@REPLACE_ME
...
echo "===================== JOB ID ================"
echo "${JOB} - https://REPLACE_ME/jobs/${JOB:2}" 
echo "===================== JOB ID ================"
...
if [[ $TOTAL_HOSTS > 0 ]]; then
    for i in `seq 1 $TOTAL_HOSTS`; do
        ...
        echo "HOSTNAME = $NAME - https://REPLACE_ME/view/$NAME"
    done
else
    JOB_HOSTNAME=`xmlstarlet sel -t -v //recipe/@system job-result`
    echo "HOSTNAME = $JOB_HOSTNAME - https://REPLACE_ME/view/$JOB_HOSTNAME"
fi
rm -Rf hostname
